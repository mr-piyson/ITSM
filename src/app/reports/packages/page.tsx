...
import { DateCellRenderer } from "../CellsRender";
import { filterParam } from "../params";
import type { ApiReportData } from "@/server/routers/MES/reports/packages";
...
export default function ReportPage() {
	...
	const { data, isLoading, isFetching, isError, error, refetch } =
		trpc.mes.packages.getPackages.useQuery({
			filter,
		});
	...
	const columnDefs = useMemo<ColDef<ApiReportData>[]>(
		() => [
			{
				headerName: "Box Code",
				field: "code",
	...
	const exportRows = useCallback(
		() =>
			gridApi?.exportDataAsCsv({
				fileName: `packages-${new Date().toISOString().split("T")[0]}.csv`,
			}),
		[gridApi],
	);
