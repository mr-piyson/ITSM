...
import { DailyAnalyticsChart } from "@/layout/MES/charts/inspections/daily-analytics-chart";
import { DefectTypeChart } from "@/layout/MES/charts/inspections/defect-type-chart";
import { GateAnalyticsChart } from "@/layout/MES/charts/inspections/gate-analytics-chart";
import {
	fromParam,
	gateParam,
	toParam,
...
import type { InspectionResult } from "@/server/routers/MES/reports/inspection-results";
			...
			<div className="p-3 border-b bg-card space-y-3 shrink-0">
				<div className="flex flex-wrap items-center gap-2">
						...
						<PopoverTrigger
							render={
								<Button
									...
									className="h-8 w-32.5 justify-between text-xs"
								>
					...
					<div className="ml-auto">
						...
						>
							<TabsList className="h-8 border border-foreground/10 ">
				...
				<Tabs value={gate} onValueChange={(val) => setGate(val)}>
					<TabsList className="w-full h-full overflow-x-auto overflow-y-hidden">
						{GATE_OPTIONS.map((opt) => (
							<TabsTrigger
								key={opt.value}
								value={opt.value}
								className="flex-1 text-xs min-w-18.75"
							>
								{opt.label}
							</TabsTrigger>
						))}
