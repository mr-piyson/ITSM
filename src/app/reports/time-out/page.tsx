...
import { PivotedReport } from "@/server/routers/MES/reports/time-out";
...
const useReportColumns = () => {
	return useMemo<ColDef<PivotedReport>[]>(() => {
		const columnDefs: ColDef<PivotedReport>[] = [
			{
				field: "panel_serial",
		...
		const gateColumns: ColDef<PivotedReport>[] = gateOrder
			.filter((id) => workstation[id])
			.map((id) => ({
				field: id.toString(),
...
export default function ReportPage() {
	...
	const { data, isLoading, isFetching, isError, error, refetch } =
		trpc.mes.timeOut.getHistory.useQuery({
			filter,
		});
	...
	const exportRows = useCallback(
		() =>
			gridApi?.exportDataAsCsv({
				fileName: `time-out-${new Date().toISOString().split("T")[0]}.csv`,
			}),
		[gridApi],
	);
