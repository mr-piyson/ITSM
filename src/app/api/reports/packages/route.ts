...
export async function GET(request: NextRequest) {
  ...
  switch (filter) {
    case "today":
      ...
      break
    case "last30days":
      sql += " WHERE DATE(p.created_at) >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)"
      // paramValue is not strictly needed for MySQL's internal date functions
      break
    case "last90days":
      sql += " WHERE DATE(p.created_at) >= DATE_SUB(CURDATE(), INTERVAL 90 DAY)"
      break
    case "1year":
      sql += " WHERE DATE(p.created_at) >= DATE_SUB(CURDATE(), INTERVAL 1 YEAR)"
      break
    case "2years":
      sql += " WHERE DATE(p.created_at) >= DATE_SUB(CURDATE(), INTERVAL 2 YEAR)"
      break
    case "3years":
      sql += " WHERE DATE(p.created_at) >= DATE_SUB(CURDATE(), INTERVAL 3 YEAR)"
      break
    case "all":
