...
const searchOptions: { value: keyof ReportData; label: string }[] = [
	{ value: "box_code", label: "Box Code" },
	{ value: "container_id", label: "Container Code" },
	{ value: "project", label: "Project" },
	{ value: "epicor_asm_part_no", label: "ASM Part No" },
	{ value: "epicor_part_no", label: "Epicor Part No" },
];
...
const BoxCellRenderer = ({ data }: { data: ReportData }) => (
	<div className="flex justify-between items-center">
		<span className="text-md font-semibold">{data.box_code}</span>
		{data.box_code && (
			<Button
				variant="outline"
				size="icon"
				...
				onClick={() =>
					window.open(
						`http://intranet.bfginternational.com:88/packages/show?id=${data.box_code}`,
						"_blank",
					)
				}
				title="Inspect Box"
...
export default function ReportPage() {
	...
	const fetchPanels = useCallback(async (): Promise<ReportData[]> => {
		const response = await axios.get(
			`http://172.18.10.40/ITSM/php/MES/shipment.php?year=${year}&month=${month}`,
		);
