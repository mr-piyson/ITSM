	...
	getPaintInspectionData: publicProcedure
		...
		.query(async ({ input }) => {
			...
			try {
				...
				const [panel]: any = await db.mes.execute(
					`
        ...
        `,
				...
				const [paintData]: any = await db.mes.execute(
					`
          SELECT 
        ...
        `,
				...
				const [checklist]: any = await db.mes.execute(
					`
        ...
        `,
				...
				// 4. Fetch Defect Data
				const [defects]: any = await db.mes.execute(
					`
