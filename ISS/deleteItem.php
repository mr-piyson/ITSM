<?php
  if ($_SERVER['REQUEST_METHOD'] === 'POST'){
    if(isset($_POST["itemID"]) && !empty($_POST["itemID"]) && isset($_POST["user"]) && !empty($_POST["user"])){
      ...
      $sql = "UPDATE `items` SET `inActive`=1 WHERE `id`=".$input1;
      if($mysqli->query($sql) === TRUE){
        $sqlLog = "INSERT INTO `changes_logs`(`userID`, `date`, `action`, `node`, `nodeID`) VALUES (".$input2.",STR_TO_DATE('".date('Y-m-d H:i:s')."', '%Y-%m-%d %H:%i:%s'),'delete','item',".$input1.")";
        $mysqli->query($sqlLog);
        include "header.php";
        ...
        die('<meta http-equiv="refresh" content="2;url=stock.php" />');
      }else{
        include "header.php";
        echo "<script>document.getElementById('headerDiv').style.display='block';</script>";
